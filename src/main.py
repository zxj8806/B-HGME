...
import traceback
from b_hgme import B_HGME

def parse_arguments():
    parser = argparse.ArgumentParser(description='B_HGME arguments')
    parser.add_argument("-m", "--mode", type=str, default="train",
                        help="B_HGME mode: preprocess,train,test")
...
def preprocess(st_data, num_nearestneighbors, lrgene_ids, cespgrn_hyperparameters, ownadjacencypath=None):
    ...
    gene_level_graph, num2gene, gene2num, grn_components = preprocessing.construct_genelevel_graph(grns, celllevel_adj,
                                                                                                   node_type="int",
                                                                                                   lrgenes=lrgene_ids)
...
def build_BHGME_pytorch(data, hyperparams=None, n_experts=100):
    ...
    cellEncoders = nn.ModuleList([models.GraphEncoder(num_cellfeatures, hidden_dim) for _ in range(n_experts)])
    geneEncoders = nn.ModuleList([
        models.SubgraphEncoder(
            num_features=num_genefeatures,
            hidden_dim=hidden_dim,
            num_vertices=num_cells,
            num_subvertices=num_genespercell
        ) for _ in range(n_experts)
    ])
    cellConcentrations = nn.ModuleList([models.GraphEncoder(num_cellfeatures, 1) for _ in range(n_experts)])

    geneConcentrations = nn.ModuleList([
        models.SubgraphEncoder(
            num_features=num_genefeatures,
            #hidden_dim=hidden_dim,
            hidden_dim=1,
    ...
    multiviewEncoder = models.MultiviewEncoder(SubgraphConcentrations=geneConcentrations, GraphConcentrations=cellConcentrations,
                                               SubgraphEncoders=geneEncoders, GraphEncoders=cellEncoders,
                                               n_experts=n_experts)
    bHGME = B_HGME(multiviewEncoder, n_experts)
    return bHGME
...
def main():
    ...
    with console.status("[cyan] B_HGME booting up ...") as status:
    ...
    if "preprocess" in mode:
        ...
        celllevel_adj, genelevel_graph, num2gene, gene2num, grns, genelevel_features, genelevel_feature_model = preprocess(
            selected_st_data, num_nearestneighbors, lrgene2id.values(), cespgrn_hyperparameters, ownadjacencypath)
    ...
    if "train" in mode:
        ...
        model = build_BHGME_pytorch(data, hyperparameters, n_experts)

        if hyperparameters["optimizer"] == "adam":
            hyperparameters["optimizer"] = torch.optim.Adam(model.parameters(), lr=0.002),

        split = hyperparameters["split"]

        trained_model, metrics_df = training.train_BHGME(model=model, data=data, hyperparameters=hyperparameters,
                                                       n_experts=n_experts)

        torch.save(trained_model.state_dict(), os.path.join(training_output_path, f'{studyname}_trained_model.pth'))
